...
// other byte intact, then re-CRCs and writes the sector.
func (d *Device) SetProfileButton(sector, index int, a ButtonAction) error {
	if index < 0 || index >= 16 {
		return fmt.Errorf("button index %d out of range", index)
	}
	enc := a.encode()
